import { Loader2, MessageCircle, Minus, Send, Users, X } from "lucide-react";
import { type FormEvent, type KeyboardEvent, useEffect, useRef, useState } from "react";
import { toast } from "sonner";
import { staffSupabase } from "@/integrations/supabase/staff-client";

type ChatMessage={id:string;staff_user_id:string;staff_name:string;message:string;created_at:string};
const timeLabel=(value:string)=>new Intl.DateTimeFormat("id-ID",{hour:"2-digit",minute:"2-digit",day:"2-digit",month:"short",timeZone:"Asia/Jakarta"}).format(new Date(value));
...
export function StaffGroupChat(){
  ...
  if(!open)return <button type="button" onClick={()=>setOpen(true)} className="fixed z-40 bottom-5 right-5 rounded-full bg-emerald text-primary-foreground shadow-xl px-4 py-3 inline-flex items-center gap-2 font-bold"><MessageCircle className="size-5"/>Chat Staff{unreadCount>0&&<span className="absolute -top-2 -right-2 min-w-6 h-6 px-1.5 rounded-full bg-red-600 text-white text-xs font-bold grid place-items-center ring-2 ring-white">{unreadCount>99?"99+":unreadCount}</span>}</button>;
  return <aside className="fixed z-40 right-3 sm:right-5 top-20 bottom-3 sm:bottom-5 w-[calc(100vw-1.5rem)] sm:w-[360px] rounded-2xl border bg-card shadow-2xl flex flex-col overflow-hidden">
    ...
    <form onSubmit={sendMessage} className="border-t bg-card p-3"><div className="flex items-end gap-2"><textarea value={draft} onChange={event=>setDraft(event.target.value)} onKeyDown={keyDown} maxLength={2000} rows={2} placeholder="Tulis pesan koordinasi..." className="min-h-[44px] max-h-28 flex-1 resize-none rounded-xl border bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"/><button type="submit" disabled={!draft.trim()||sending} className="size-11 shrink-0 rounded-xl bg-accent text-primary-foreground grid place-items-center disabled:opacity-50">{sending?<Loader2 className="size-4 animate-spin"/>:<Send className="size-4"/>}</button></div><div className="mt-1 text-[10px] text-muted-foreground">Enter untuk kirim · Shift+Enter untuk baris baru</div></form>
  </aside>;
}
