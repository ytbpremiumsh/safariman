...
type StaffReview={reviewer_name:string;decision:Status;reviewed_at:string;updated_at:string};
...
function StaffDashboard() {
  ...
  const decide=async(status:Status)=>{ if(!detail)return; setBusy(true); const {data,error}=await supabase.functions.invoke("staff-essay",{body:{action:"update_status",participant_id:detail.id,status}}); setBusy(false); if(error){toast.error(error.message);return;} const next={...detail,status,staff_review:data?.review??detail.staff_review};setRows(v=>v.map(r=>r.id===detail.id?next:r));setDetail(next);toast.success("Keputusan berhasil disimpan. Hasil belum dipublikasikan."); };
