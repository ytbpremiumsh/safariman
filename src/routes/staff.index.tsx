  ...
  return <div className="min-h-screen bg-secondary/30">
    ...
    <main className="max-w-7xl mx-auto p-4 sm:p-6 space-y-5">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <button onClick={()=>setFilter("interview")} className={`rounded-2xl border p-4 text-left transition ${filter==="interview"?"border-emerald bg-emerald/10 ring-2 ring-emerald/20":"bg-card hover:border-emerald/50"}`}><div className="text-xs font-semibold text-muted-foreground">Lolos Tahap Selanjutnya</div><div className="mt-2 text-3xl font-bold text-emerald">{counts.interview}</div></button>
        <button onClick={()=>setFilter("rejected")} className={`rounded-2xl border p-4 text-left transition ${filter==="rejected"?"border-destructive bg-destructive/10 ring-2 ring-destructive/20":"bg-card hover:border-destructive/50"}`}><div className="text-xs font-semibold text-muted-foreground">Tidak Lolos</div><div className="mt-2 text-3xl font-bold text-destructive">{counts.rejected}</div></button>
        <button onClick={()=>setFilter("reviewed")} className={`rounded-2xl border p-4 text-left transition ${filter==="reviewed"?"border-amber-400 bg-amber-50 ring-2 ring-amber-200":"bg-card hover:border-amber-400"}`}><div className="text-xs font-semibold text-muted-foreground">Belum Diputuskan</div><div className="mt-2 text-3xl font-bold text-amber-700">{counts.reviewed}</div></button>
      </div>
      <div className="bg-card border rounded-2xl p-4 grid sm:grid-cols-[minmax(0,1fr),220px,240px] gap-3">
        <div className="relative"><Search className="absolute size-4 left-3 top-3 text-muted-foreground"/><Input className="pl-9" placeholder="Cari nama, token, email, kota, atau pengoreksi" value={q} onChange={e=>setQ(e.target.value)}/></div>
