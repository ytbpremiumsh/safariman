...
import { CalendarClock, CheckCircle2, FileText, Loader2, LogOut, Search, ShieldCheck, XCircle } from "lucide-react";
...
type StaffReview={reviewer_name:string;decision:Status;scores:ReviewScores;total_score:number;reviewer_notes:string|null;criteria_checks:ReviewChecks|null;reviewed_at:string;updated_at:string};
...
function StaffDashboard() {
  ...
  return <div className="min-h-screen bg-secondary/30">
    ...
    <Dialog open={!!detail} onOpenChange={o=>!o&&setDetail(null)}><DialogContent className="max-w-5xl max-h-[92vh] overflow-y-auto">{detail&&<><DialogHeader><DialogTitle>{detail.full_name} · {detail.registration_code}</DialogTitle><div className="flex items-center gap-1.5 text-xs text-muted-foreground"><CalendarClock className="size-3.5"/><span>Dikirim: {formatSubmissionDate(detail.updated_at)}</span></div></DialogHeader><ManualEssayReview answers={{essay_1:detail.essay_worthy,essay_2:detail.essay_dream,essay_3:detail.essay_contribution,case_1:detail.case_study_1,case_2:detail.case_study_2,case_3:detail.case_study_3,case_4:detail.case_study_4,case_5:detail.case_study_5,case_6:detail.case_study_6,case_7:detail.case_study_7}} initialScores={detail.staff_review?.scores} initialChecks={detail.staff_review?.criteria_checks} initialNotes={detail.staff_review?.reviewer_notes} currentDecision={detail.status} busy={busy} analyzing={analyzing} onAnalyze={analyze} onSave={decide} onReset={resetReview}/></>}</DialogContent></Dialog>
  </div>;
}
