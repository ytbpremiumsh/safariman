          ...
          <div className="flex items-start justify-between gap-4 rounded-xl border border-border bg-secondary/30 p-4">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed">
                ...
                pembayaran Mayar — peserta langsung mendapat status Lolos dan akses dokumen.
              </p>
            </div>
            <button
              type="button"
              role="switch"
              aria-checked={paidEnabled}
              onClick={() => setPaidEnabled((v) => !v)}
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors ${
                paidEnabled ? "bg-emerald" : "bg-muted"
              }`}
            >
              <span
                className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition-transform ${
                  paidEnabled ? "translate-x-5" : "translate-x-0"
                }`}
              />
            </button>
          </div>

          <div className={paidEnabled ? "" : "opacity-50 pointer-events-none"}>
